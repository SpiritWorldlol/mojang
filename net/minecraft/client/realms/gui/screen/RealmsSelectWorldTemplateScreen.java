...
public class RealmsSelectWorldTemplateScreen extends RealmsScreen {
   ...
   public void render(DrawContext context, int mouseX, int mouseY, float delta) {
      ...
      context.drawCenteredTextWithShadow(this.textRenderer, (Text)this.title, this.width / 2, 13, 16777215);
      if (this.displayWarning) {
         ...
         for(k = 0; k < lvs.length; ++k) {
            ...
            context.drawCenteredTextWithShadow(this.textRenderer, (Text)lv, this.width / 2, row(-1 + k), m);
         }
      }
   ...
   private void renderMessages(DrawContext context, int x, int y, List messages) {
      for(int k = 0; k < messages.size(); ++k) {
         ...
         for(Iterator var10 = lv.segments.iterator(); var10.hasNext(); n = p) {
            ...
            p = context.drawTextWithShadow(this.textRenderer, lv2.renderedText(), n, l, o);
   ...
   private class WorldTemplateObjectSelectionListEntry extends AlwaysSelectedEntryListWidget.Entry {
      ...
      private void renderWorldTemplateItem(DrawContext context, WorldTemplate template, int x, int y, int mouseX, int mouseY) {
         int m = x + 45 + 20;
         context.drawText(RealmsSelectWorldTemplateScreen.this.textRenderer, template.name, m, y + 2, 16777215, false);
         context.drawText(RealmsSelectWorldTemplateScreen.this.textRenderer, template.author, m, y + 15, 7105644, false);
         context.drawText(RealmsSelectWorldTemplateScreen.this.textRenderer, template.version, m + 227 - RealmsSelectWorldTemplateScreen.this.textRenderer.getWidth(template.version), y + 1, 7105644, false);
         if (!"".equals(template.link) || !"".equals(template.trailer) || !"".equals(template.recommendedPlayers)) {
            this.drawIcons(context, m - 1, y + 25, mouseX, mouseY, template.link, template.trailer, template.recommendedPlayers);
         }
      ...
      private void drawIcons(DrawContext context, int x, int y, int mouseX, int mouseY, String link, String trailer, String recommendedPlayers) {
         ...
         if (!bl3) {
            ...
            context.drawTexture(RealmsSelectWorldTemplateScreen.LINK_ICONS, x + m, y, f, 0.0F, 15, 15, 30, 15);
         }

         if (!"".equals(trailer)) {
            int n = x + m + (bl3 ? 0 : 17);
            float g = bl2 ? 15.0F : 0.0F;
            context.drawTexture(RealmsSelectWorldTemplateScreen.TRAILER_ICONS, n, y, g, 0.0F, 15, 15, 30, 15);
         }
