...
public class SystemToast implements Toast {
   ...
   public Toast.Visibility draw(DrawContext context, ToastManager manager, long startTime) {
      ...
      if (i == 160 && this.lines.size() <= 1) {
         ...
         for(int n = 28; n < j - m; n += 10) {
            this.drawPart(context, manager, i, 16, n, Math.min(16, j - n - m));
         }

         this.drawPart(context, manager, i, 32 - m, j - m, m);
      }

      if (this.lines == null) {
         context.drawText(manager.getClient().textRenderer, (Text)this.title, 18, 12, -256, false);
      } else {
         context.drawText(manager.getClient().textRenderer, (Text)this.title, 18, 7, -256, false);

         for(j = 0; j < this.lines.size(); ++j) {
            context.drawText(manager.getClient().textRenderer, (OrderedText)((OrderedText)this.lines.get(j)), 18, 18 + j * 12, -1, false);
         }
      }
   ...
   private void drawPart(DrawContext context, ToastManager manager, int width, int textureV, int y, int height) {
      ...
      context.drawTexture(TEXTURE, 0, y, 0, 64 + textureV, m, height);

      for(int o = m; o < width - n; o += 64) {
         context.drawTexture(TEXTURE, o, y, 32, 64 + textureV, Math.min(64, width - o - n), height);
      }

      context.drawTexture(TEXTURE, width - n, y, 160 - n, 64 + textureV, n, height);
   }
