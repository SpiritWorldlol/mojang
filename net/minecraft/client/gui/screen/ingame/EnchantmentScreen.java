...
public class EnchantmentScreen extends HandledScreen {
   ...
   protected void drawBackground(DrawContext context, float delta, int mouseX, int mouseY) {
      ...
      context.getMatrices().translate((1.0F - h) * 0.2F, (1.0F - h) * 0.1F, (1.0F - h) * 0.25F);
      float n = -(1.0F - h) * 90.0F - 90.0F;
      context.getMatrices().multiply(RotationAxis.POSITIVE_Y.rotationDegrees(n));
      ...
      this.BOOK_MODEL.render(context.getMatrices(), lv, LightmapTextureManager.MAX_LIGHT_COORDINATE, OverlayTexture.DEFAULT_UV, 1.0F, 1.0F, 1.0F, 1.0F);
      context.draw();
      ...
      for(int r = 0; r < 3; ++r) {
         ...
         if (u == 0) {
            context.drawTexture(TEXTURE, s, l + 14 + 19 * r, 0, 185, 108, 19);
         } else {
            String string = "" + u;
            int v = 86 - this.textRenderer.getWidth(string);
            StringVisitable lv2 = EnchantingPhrases.getInstance().generatePhrase(this.textRenderer, v);
            int w = 6839882;
            if ((q < r + 1 || this.client.player.experienceLevel < u) && !this.client.player.getAbilities().creativeMode) {
               context.drawTexture(TEXTURE, s, l + 14 + 19 * r, 0, 185, 108, 19);
               context.drawTexture(TEXTURE, s + 1, l + 15 + 19 * r, 16 * r, 239, 16, 16);
               context.drawTextWrapped(this.textRenderer, lv2, t, l + 16 + 19 * r, v, (w & 16711422) >> 1);
               w = 4226832;
            } else {
               int x = mouseX - (k + 60);
               int y = mouseY - (l + 14 + 19 * r);
               if (x >= 0 && y >= 0 && x < 108 && y < 19) {
                  context.drawTexture(TEXTURE, s, l + 14 + 19 * r, 0, 204, 108, 19);
                  w = 16777088;
               } else {
                  context.drawTexture(TEXTURE, s, l + 14 + 19 * r, 0, 166, 108, 19);
               }

               context.drawTexture(TEXTURE, s + 1, l + 15 + 19 * r, 16 * r, 223, 16, 16);
               context.drawTextWrapped(this.textRenderer, lv2, t, l + 16 + 19 * r, v, w);
               w = 8453920;
            }

            context.drawTextWithShadow(this.textRenderer, string, t + 86 - this.textRenderer.getWidth(string), l + 16 + 19 * r + 7, w);
         }
      }
