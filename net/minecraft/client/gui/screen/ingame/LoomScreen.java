...
public class LoomScreen extends HandledScreen {
   ...
   protected void drawBackground(DrawContext context, float delta, int mouseX, int mouseY) {
      ...
      if (!lv.hasStack()) {
         context.drawTexture(TEXTURE, k + lv.x, l + lv.y, this.backgroundWidth, 0, 16, 16);
      }

      if (!lv2.hasStack()) {
         context.drawTexture(TEXTURE, k + lv2.x, l + lv2.y, this.backgroundWidth + 16, 0, 16, 16);
      }

      if (!lv3.hasStack()) {
         context.drawTexture(TEXTURE, k + lv3.x, l + lv3.y, this.backgroundWidth + 32, 0, 16, 16);
      }

      int m = (int)(41.0F * this.scrollPosition);
      context.drawTexture(TEXTURE, k + 119, l + 13 + m, 232 + (this.canApplyDyePattern ? 0 : 12), 0, 12, 15);
      DiffuseLighting.disableGuiDepthLighting();
      if (this.bannerPatterns != null && !this.hasTooManyPatterns) {
         context.getMatrices().push();
         context.getMatrices().translate((float)(k + 139), (float)(l + 52), 0.0F);
         ...
         BannerBlockEntityRenderer.renderCanvas(context.getMatrices(), context.getVertexConsumers(), 15728880, OverlayTexture.DEFAULT_UV, this.bannerField, ModelLoader.BANNER_BASE, true, this.bannerPatterns);
         context.getMatrices().pop();
         context.draw();
      } else if (this.hasTooManyPatterns) {
         context.drawTexture(TEXTURE, k + lv4.x - 2, l + lv4.y - 2, this.backgroundWidth, 17, 17, 16);
      }
      ...
      if (this.canApplyDyePattern) {
         ...
         for(int p = 0; p < 4; ++p) {
            for(int q = 0; q < 4; ++q) {
               ...
               context.drawTexture(TEXTURE, t, u, 0, v, 14, 14);
               this.drawBanner(context, (RegistryEntry)list.get(s), t, u);
            }
         }
      }
   ...
   private void drawBanner(DrawContext context, RegistryEntry pattern, int x, int y) {
      NbtCompound lv = new NbtCompound();
      NbtList lv2 = (new BannerPattern.Patterns()).add(BannerPatterns.BASE, DyeColor.GRAY).add(pattern, DyeColor.WHITE).toNbt();
      lv.put("Patterns", lv2);
      ...
      BannerBlockEntityRenderer.renderCanvas(lv4, context.getVertexConsumers(), 15728880, OverlayTexture.DEFAULT_UV, this.bannerField, ModelLoader.BANNER_BASE, true, list);
      lv4.pop();
      context.draw();
   }
