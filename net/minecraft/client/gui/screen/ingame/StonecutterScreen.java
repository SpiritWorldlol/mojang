...
public class StonecutterScreen extends HandledScreen {
   ...
   protected void drawBackground(DrawContext context, float delta, int mouseX, int mouseY) {
      ...
      context.drawTexture(TEXTURE, k + 119, l + 15 + m, 176 + (this.shouldScroll() ? 0 : 12), 0, 12, 15);
      int n = this.x + 52;
      int o = this.y + 14;
      ...
      if (this.canCraft) {
         ...
         for(int n = this.scrollOffset; n < m && n < ((StonecutterScreenHandler)this.handler).getAvailableRecipeCount(); ++n) {
            ...
            if (x >= p && x < p + 16 && y >= q && y < q + 18) {
               context.drawItemTooltip(this.textRenderer, ((StonecuttingRecipe)list.get(n)).getOutput(this.client.world.getRegistryManager()), x, y);
            }
         }
      }
   ...
   private void renderRecipeIcons(DrawContext context, int x, int y, int scrollOffset) {
      ...
      for(int l = this.scrollOffset; l < scrollOffset && l < ((StonecutterScreenHandler)this.handler).getAvailableRecipeCount(); ++l) {
         ...
         int o = m / 4;
         int p = y + o * 18 + 2;
         context.drawItem(((StonecuttingRecipe)list.get(l)).getOutput(this.client.world.getRegistryManager()), n, p);
      }
