   ...
   class GeneralStatsListWidget extends AlwaysSelectedEntryListWidget {
      ...
      private class Entry extends AlwaysSelectedEntryListWidget.Entry {
         ...
         public void render(DrawContext context, int index, int y, int x, int entryWidth, int entryHeight, int mouseX, int mouseY, boolean hovered, float tickDelta) {
            ...
            context.drawTextWithShadow(StatsScreen.this.textRenderer, string, x + 2 + 213 - StatsScreen.this.textRenderer.getWidth(string), y + 1, index % 2 == 0 ? 16777215 : 9474192);
         }
   ...
   private class ItemStatsListWidget extends AlwaysSelectedEntryListWidget {
      ...
      protected void renderHeader(DrawContext context, int x, int y) {
         ...
         for(k = 0; k < this.HEADER_ICON_SPRITE_INDICES.length; ++k) {
            StatsScreen.this.renderIcon(context, x + StatsScreen.this.getColumnX(k) - 18, y + 1, 0, this.selectedHeaderColumn == k ? 0 : 18);
         }
         ...
         if (this.selectedStatType != null) {
            ...
            StatsScreen.this.renderIcon(context, x + k, y + 1, 18 * l, 0);
         }

         for(k = 0; k < this.HEADER_ICON_SPRITE_INDICES.length; ++k) {
            l = this.selectedHeaderColumn == k ? 1 : 0;
            StatsScreen.this.renderIcon(context, x + StatsScreen.this.getColumnX(k) - 18 + l, y + 1 + l, 18 * this.HEADER_ICON_SPRITE_INDICES[k], 18);
         }
      ...
      protected void renderDecorations(DrawContext context, int mouseX, int mouseY) {
         if (mouseY >= this.top && mouseY <= this.bottom) {
            ...
            if (lv != null) {
               ...
               Item lv2 = lv.getItem();
               this.render(context, this.getText(lv2), mouseX, mouseY);
            } else {
               Text lv3 = null;
               int l = mouseX - k;
               ...
               this.render(context, lv3, mouseX, mouseY);
            }

         }
      }

      protected void render(DrawContext arg, @Nullable Text text, int mouseX, int mouseY) {
         if (text != null) {
            int k = mouseX + 12;
            int l = mouseY - 12;
            int m = StatsScreen.this.textRenderer.getWidth((StringVisitable)text);
            arg.fillGradient(k - 3, l - 3, k + m + 3, l + 8 + 3, -1073741824, -1073741824);
            arg.getMatrices().push();
            arg.getMatrices().translate(0.0F, 0.0F, 400.0F);
            arg.drawTextWithShadow(StatsScreen.this.textRenderer, (Text)text, k, l, -1);
            arg.getMatrices().pop();
         }
      }
      ...
      class Entry extends AlwaysSelectedEntryListWidget.Entry {
         ...
         public void render(DrawContext context, int index, int y, int x, int entryWidth, int entryHeight, int mouseX, int mouseY, boolean hovered, float tickDelta) {
            ...
            for(p = 0; p < StatsScreen.this.itemStats.blockStatTypes.size(); ++p) {
               ...
               this.render(context, lv, x + StatsScreen.this.getColumnX(p), y, index % 2 == 0);
            }

            for(p = 0; p < StatsScreen.this.itemStats.itemStatTypes.size(); ++p) {
               this.render(context, ((StatType)StatsScreen.this.itemStats.itemStatTypes.get(p)).getOrCreateStat(this.item), x + StatsScreen.this.getColumnX(p + StatsScreen.this.itemStats.blockStatTypes.size()), y, index % 2 == 0);
            }

         }

         protected void render(DrawContext arg, @Nullable Stat stat, int x, int y, boolean white) {
            String string = stat == null ? "-" : stat.format(StatsScreen.this.statHandler.getStat(stat));
            arg.drawTextWithShadow(StatsScreen.this.textRenderer, string, x - StatsScreen.this.textRenderer.getWidth(string), y + 5, white ? 16777215 : 9474192);
         }
   ...
   private class EntityStatsListWidget extends AlwaysSelectedEntryListWidget {
      ...
      class Entry extends AlwaysSelectedEntryListWidget.Entry {
         ...
         public void render(DrawContext context, int index, int y, int x, int entryWidth, int entryHeight, int mouseX, int mouseY, boolean hovered, float tickDelta) {
            ...
            int var10004 = y + 1;
            Objects.requireNonNull(StatsScreen.this.textRenderer);
            context.drawTextWithShadow(var10001, var10002, var10003, var10004 + 9, this.killedAny ? 9474192 : 6316128);
            var10001 = StatsScreen.this.textRenderer;
            var10002 = this.killedByText;
            var10003 = x + 2 + 10;
            var10004 = y + 1;
            Objects.requireNonNull(StatsScreen.this.textRenderer);
            context.drawTextWithShadow(var10001, var10002, var10003, var10004 + 9 * 2, this.killedByAny ? 9474192 : 6316128);
         }
