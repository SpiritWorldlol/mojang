...
public class RecipeAlternativesWidget implements Drawable, Element {
   ...
   public void render(DrawContext context, int mouseX, int mouseY, float delta) {
      if (this.visible) {
         ...
         int n = true;
         context.drawNineSlicedTexture(BACKGROUND_TEXTURE, this.buttonX, this.buttonY, l * 25 + 8, m * 25 + 8, 4, 32, 32, 82, 208);
   ...
   private class AlternativeButtonWidget extends ClickableWidget implements RecipeGridAligner {
      ...
      public void renderButton(DrawContext context, int mouseX, int mouseY, float delta) {
         ...
         context.drawTexture(RecipeAlternativesWidget.BACKGROUND_TEXTURE, this.getX(), this.getY(), k, l, this.width, this.height);
         context.getMatrices().push();
         ...
         while(var7.hasNext()) {
            ...
            context.getMatrices().push();
            context.getMatrices().translate((double)lv.y, (double)lv.x, 0.0);
            context.getMatrices().scale(0.375F, 0.375F, 1.0F);
            context.getMatrices().translate(-8.0, -8.0, 0.0);
            context.drawItem(lv.stacks[MathHelper.floor(RecipeAlternativesWidget.this.time / 30.0F) % lv.stacks.length], 0, 0);
            context.getMatrices().pop();
         }
