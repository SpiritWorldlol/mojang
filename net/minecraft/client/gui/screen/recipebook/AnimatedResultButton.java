...
public class AnimatedResultButton extends ClickableWidget {
   ...
   public void renderButton(DrawContext context, int mouseX, int mouseY, float delta) {
      ...
      context.drawTexture(BACKGROUND_TEXTURE, this.getX(), this.getY(), k, l, this.width, this.height);
      ...
      if (this.resultCollection.hasSingleOutput() && this.getResults().size() > 1) {
         context.drawItem(lv2, this.getX() + m + 1, this.getY() + m + 1, 0, 10);
         --m;
      }

      context.drawItemWithoutEntity(lv2, this.getX() + m, this.getY() + m);
      if (bl) {
   ...
   public List getTooltip() {
      ...
      List list = Lists.newArrayList(Screen.getTooltipFromItem(MinecraftClient.getInstance(), lv));
      if (this.resultCollection.getResults(this.recipeBook.isFilteringCraftable(this.craftingScreenHandler)).size() > 1) {
