...
public class AdvancementWidget {
   ...
   public void renderWidgets(DrawContext context, int x, int y) {
      if (!this.display.isHidden() || this.progress != null && this.progress.isDone()) {
         ...
         context.drawTexture(WIDGETS_TEXTURE, x + this.x + 3, y + this.y, this.display.getFrame().getTextureV(), 128 + lv.getSpriteIndex() * 26, 26, 26);
   ...
   public void drawTooltip(DrawContext context, int originX, int originY, float alpha, int x, int y) {
      ...
      if (!this.description.isEmpty()) {
         if (bl2) {
            context.drawNineSlicedTexture(WIDGETS_TEXTURE, q, p + 26 - r, this.width, r, 10, 200, 26, 0, 52);
         } else {
            context.drawNineSlicedTexture(WIDGETS_TEXTURE, q, p, this.width, r, 10, 200, 26, 0, 52);
         }
      }

      context.drawTexture(WIDGETS_TEXTURE, q, p, 0, lv.getSpriteIndex() * 26, n, 26);
      context.drawTexture(WIDGETS_TEXTURE, q + n, p, 200 - o, lv2.getSpriteIndex() * 26, o, 26);
      context.drawTexture(WIDGETS_TEXTURE, originX + this.x + 3, originY + this.y, this.display.getFrame().getTextureV(), 128 + lv3.getSpriteIndex() * 26, 26, 26);
      if (bl) {
         context.drawTextWithShadow(this.client.textRenderer, (OrderedText)this.title, q + 5, originY + this.y + 9, -1);
         if (string != null) {
            context.drawTextWithShadow(this.client.textRenderer, (String)string, originX + this.x - m, originY + this.y + 9, -1);
         }
      } else {
         context.drawTextWithShadow(this.client.textRenderer, (OrderedText)this.title, originX + this.x + 32, originY + this.y + 9, -1);
         if (string != null) {
            context.drawTextWithShadow(this.client.textRenderer, (String)string, originX + this.x + this.width - m - 5, originY + this.y + 9, -1);
         }
      }
      ...
      if (bl2) {
         for(s = 0; s < this.description.size(); ++s) {
            ...
            var10004 = p + 26 - r + 7;
            Objects.requireNonNull(this.client.textRenderer);
            context.drawText(var21, var22, var10003, var10004 + s * 9, -5592406, false);
         }
      } else {
         for(s = 0; s < this.description.size(); ++s) {
            var21 = this.client.textRenderer;
