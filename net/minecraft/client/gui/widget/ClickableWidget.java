...
public abstract class ClickableWidget implements Drawable, Element, Widget, Selectable {
   ...
   protected static void drawScrollableText(DrawContext context, TextRenderer textRenderer, Text text, int left, int top, int right, int bottom, int color) {
      ...
      if (n > p) {
         ...
         context.drawTextWithShadow(textRenderer, text, left - (int)g, o, color);
         context.disableScissor();
      } else {
         context.drawCenteredTextWithShadow(textRenderer, text, (left + right) / 2, o, color);
      }
   ...
   protected void drawScrollableText(DrawContext context, TextRenderer textRenderer, int xMargin, int color) {
      ...
      drawScrollableText(context, textRenderer, this.getMessage(), k, this.getY(), l, this.getY() + this.getHeight(), color);
   }
   ...
   public void drawTexture(DrawContext context, Identifier texture, int x, int y, int u, int v, int hoveredVOffset, int width, int height, int textureWidth, int textureHeight) {
      ...
      context.drawTexture(texture, x, y, (float)u, (float)r, width, height, textureWidth, textureHeight);
   }
