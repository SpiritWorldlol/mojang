...
public class PlayerListHud {
   ...
   public void render(DrawContext context, int scaledWindowWidth, Scoreboard scoreboard, @Nullable ScoreboardObjective objective) {
      ...
      if (list2 != null) {
         ...
         context.fill(var33, var10002, var10003, r + var10005 * 9, Integer.MIN_VALUE);

         for(var35 = list2.iterator(); var35.hasNext(); r += 9) {
            lv3 = (OrderedText)var35.next();
            t = this.client.textRenderer.getWidth(lv3);
            context.drawTextWithShadow(this.client.textRenderer, (OrderedText)lv3, scaledWindowWidth / 2 - t / 2, r, -1);
            Objects.requireNonNull(this.client.textRenderer);
         }
      ...
      context.fill(scaledWindowWidth / 2 - s / 2 - 1, r - 1, scaledWindowWidth / 2 + s / 2 + 1, r + n * 9, Integer.MIN_VALUE);
      ...
      for(int v = 0; v < m; ++v) {
         ...
         if (v < list.size()) {
            ...
            if (bl) {
               ...
               PlayerSkinDrawer.draw(context, lv4.getSkinTexture(), x, y, 8, bl3, bl2);
               x += 9;
            }

            context.drawTextWithShadow(this.client.textRenderer, this.getPlayerName(lv4), x, y, lv4.getGameMode() == GameMode.SPECTATOR ? -1862270977 : -1);
            if (objective != null && lv4.getGameMode() != GameMode.SPECTATOR) {
               int z = x + j + 1;
               int aa = z + o;
               if (aa - z > 5) {
                  this.renderScoreboardObjective(objective, y, gameProfile.getName(), z, aa, gameProfile.getId(), context);
               }
            }

            this.renderLatencyIcon(context, p, x - (bl ? 9 : 0), y, lv4);
         }
      }
      ...
      if (list3 != null) {
         ...
         for(Iterator var38 = list3.iterator(); var38.hasNext(); r += 9) {
            ...
            context.drawTextWithShadow(this.client.textRenderer, (OrderedText)lv6, scaledWindowWidth / 2 - w / 2, r, -1);
            Objects.requireNonNull(this.client.textRenderer);
         }
      }
   ...
   protected void renderLatencyIcon(DrawContext context, int width, int x, int y, PlayerListEntry entry) {
      ...
      context.drawTexture(ICONS_TEXTURE, x + width - 11, y, 0, 176 + m * 8, 10, 8);
   ...
   private void renderScoreboardObjective(ScoreboardObjective objective, int y, String player, int left, int right, UUID uuid, DrawContext context) {
      ...
      if (objective.getRenderType() == ScoreboardCriterion.RenderType.HEARTS) {
         this.renderHearts(y, left, right, uuid, context, l);
      } else {
         String string2 = Formatting.YELLOW + l;
         context.drawTextWithShadow(this.client.textRenderer, string2, right - this.client.textRenderer.getWidth(string2), y, 16777215);
      }
   }
   ...
   private void renderHearts(int y, int left, int right, UUID uuid, DrawContext context, int score) {
      ...
      if (m > 0) {
         ...
         if (o <= 3) {
            ...
            context.drawTextWithShadow(this.client.textRenderer, string, (right + left - this.client.textRenderer.getWidth(string)) / 2, y, p);
         } else {
            int q;
            for(q = m; q < n; ++q) {
               context.drawTexture(ICONS_TEXTURE, left + q * o, y, bl ? 25 : 16, 0, 9, 9);
            }
            ...
            for(q = 0; q < m; ++q) {
               ...
               if (bl) {
                  if (q * 2 + 1 < lv.getPrevScore()) {
                     context.drawTexture(ICONS_TEXTURE, left + q * o, y, 70, 0, 9, 9);
                  }

                  if (q * 2 + 1 == lv.getPrevScore()) {
                     context.drawTexture(ICONS_TEXTURE, left + q * o, y, 79, 0, 9, 9);
                  }
               }

               if (q * 2 + 1 < score) {
                  context.drawTexture(ICONS_TEXTURE, left + q * o, y, q >= 10 ? 160 : 52, 0, 9, 9);
               }

               if (q * 2 + 1 == score) {
                  context.drawTexture(ICONS_TEXTURE, left + q * o, y, q >= 10 ? 169 : 61, 0, 9, 9);
               }
            }
