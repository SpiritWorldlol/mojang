...
public class WorldRenderer implements SynchronousResourceReloader, AutoCloseable {
   ...
   public void render(MatrixStack matrices, float tickDelta, long limitTime, boolean renderBlockOutline, Camera camera, GameRenderer gameRenderer, LightmapTextureManager lightmapTextureManager, Matrix4f positionMatrix) {
      ...
      if (this.shouldCaptureFrustum) {
         this.captureFrustum(matrix4f2, positionMatrix, lv2.x, lv2.y, lv2.z, bl2 ? new Frustum(matrix4f2, positionMatrix) : lv3);
      ...
      lv.swap("sky");
      RenderSystem.setShader(GameRenderer::getPositionProgram);
      this.renderSky(matrices, positionMatrix, tickDelta, camera, bl3, () -> {
         BackgroundRenderer.applyFog(camera, BackgroundRenderer.FogType.FOG_SKY, h, bl3, tickDelta);
      });
      lv.swap("fog");
      BackgroundRenderer.applyFog(camera, BackgroundRenderer.FogType.FOG_TERRAIN, Math.max(h, 32.0F), bl3, tickDelta);
      ...
      Iterator var25 = this.world.getEntities().iterator();
               ...
               do {
                  do {
                     if (!var25.hasNext()) {
                        ...
                        ObjectListIterator var39 = this.chunkInfos.iterator();
                        ...
                        while(true) {
                           ...
                           do {
                              if (!var39.hasNext()) {
                                 synchronized(this.noCullingBlockEntities) {
                                    Iterator var43 = this.noCullingBlockEntities.iterator();

                                    while(var43.hasNext()) {
                                       BlockEntity lv15 = (BlockEntity)var43.next();
                                 ...
                                 ObjectIterator var40 = this.blockBreakingProgressions.long2ObjectEntrySet().iterator();

                                 while(var40.hasNext()) {
                                    Long2ObjectMap.Entry entry = (Long2ObjectMap.Entry)var40.next();
                              ...
                              ChunkInfo lv9 = (ChunkInfo)var39.next();
                           ...
                           while(var50.hasNext()) {
                              BlockEntity lv10 = (BlockEntity)var50.next();
   ...
   private void updateChunks(Camera camera) {
      ...
      List list = Lists.newArrayList();
      ObjectListIterator var6 = this.chunkInfos.iterator();

      while(true) {
         ChunkBuilder.BuiltChunk lv5;
         ChunkSectionPos lv6;
         do {
            do {
               if (!var6.hasNext()) {
                  ...
                  while(var12.hasNext()) {
                     ChunkBuilder.BuiltChunk lv8 = (ChunkBuilder.BuiltChunk)var12.next();
                     lv8.scheduleRebuild(this.chunkBuilder, lv2);
                     lv8.cancelRebuild();
                  }
               ...
               ChunkInfo lv4 = (ChunkInfo)var6.next();
               lv5 = lv4.chunk;
               lv6 = ChunkSectionPos.from(lv5.getOrigin());
            } while(!lv5.needsRebuild());
         } while(!lv.isLightingEnabled(lv6));
         ...
         if (this.client.options.getChunkBuilderMode().getValue() != ChunkBuilderMode.NEARBY) {
            if (this.client.options.getChunkBuilderMode().getValue() == ChunkBuilderMode.PLAYER_AFFECTED) {
               bl = lv5.needsImportantRebuild();
            }
         } else {
            BlockPos lv7 = lv5.getOrigin().add(8, 8, 8);
            bl = lv7.getSquaredDistance(lv3) < 768.0 || lv5.needsImportantRebuild();
         }

         if (bl) {
            this.client.getProfiler().push("build_near_sync");
            this.chunkBuilder.rebuild(lv5, lv2);
            lv5.cancelRebuild();
            this.client.getProfiler().pop();
         } else {
            list.add(lv5);
