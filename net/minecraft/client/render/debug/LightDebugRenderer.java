...
public class LightDebugRenderer implements DebugRenderer.Renderer {
   ...
   public void render(MatrixStack matrices, VertexConsumerProvider vertexConsumers, double cameraX, double cameraY, double cameraZ) {
      ...
      drawFaces(matrices, this.data.readyShape, this.data.minSectionPos, lv, cameraX, cameraY, cameraZ, READY_SHAPE_COLOR);
      drawFaces(matrices, this.data.shape, this.data.minSectionPos, lv, cameraX, cameraY, cameraZ, DEFAULT_SHAPE_COLOR);
   }
   ...
   private static void drawFaces(MatrixStack matrices, VoxelSet shape, ChunkSectionPos sectionPos, VertexConsumer vertexConsumer, double cameraX, double cameraY, double cameraZ, Vector4f color) {
      shape.forEachDirection((direction, offsetX, offsetY, offsetZ) -> {
         ...
         drawFace(matrices, vertexConsumer, direction, cameraX, cameraY, cameraZ, l, m, n, color);
      });
   }
   ...
   private static void drawEdges(MatrixStack matrices, VoxelSet shape, ChunkSectionPos sectionPos, VertexConsumerProvider vertexConsumers, double cameraX, double cameraY, double cameraZ, Vector4f color) {
      shape.forEachEdge((x1, y1, z1, x2, y2, z2) -> {
         ...
         drawEdge(matrices, lv, cameraX, cameraY, cameraZ, o, p, q, r, s, t, color);
      }, true);
   }
   ...
   private static void drawFace(MatrixStack matrices, VertexConsumer vertexConsumer, Direction direction, double cameraX, double cameraY, double cameraZ, int x, int y, int z, Vector4f color) {
      ...
      switch (direction) {
         case DOWN:
            vertexConsumer.vertex(matrix4f, g, h, l).color(p, q, r, s).next();
            vertexConsumer.vertex(matrix4f, m, h, l).color(p, q, r, s).next();
            vertexConsumer.vertex(matrix4f, m, h, o).color(p, q, r, s).next();
            vertexConsumer.vertex(matrix4f, g, h, o).color(p, q, r, s).next();
            break;
         case UP:
            vertexConsumer.vertex(matrix4f, g, n, l).color(p, q, r, s).next();
            vertexConsumer.vertex(matrix4f, g, n, o).color(p, q, r, s).next();
            vertexConsumer.vertex(matrix4f, m, n, o).color(p, q, r, s).next();
            vertexConsumer.vertex(matrix4f, m, n, l).color(p, q, r, s).next();
            break;
         case NORTH:
            vertexConsumer.vertex(matrix4f, g, h, l).color(p, q, r, s).next();
            vertexConsumer.vertex(matrix4f, g, n, l).color(p, q, r, s).next();
   ...
   private static void drawEdge(MatrixStack matrices, VertexConsumer vertexConsumer, double cameraX, double cameraY, double cameraZ, int x1, int y1, int z1, int x2, int y2, int z, Vector4f color) {
      ...
      Matrix4f matrix4f = matrices.peek().getPositionMatrix();
      vertexConsumer.vertex(matrix4f, g, h, o).color(color.x(), color.y(), color.z(), 1.0F).next();
      vertexConsumer.vertex(matrix4f, p, q, r).color(color.x(), color.y(), color.z(), 1.0F).next();
   }
         ...
         for(int k = 0; k < j; ++k) {
            for(int l = 0; l < j; ++l) {
               for(int m = 0; m < j; ++m) {
                  ...
                  } else if (lv2 == LightStorage.Status.LIGHT_ONLY) {
                     this.shape.set(m, l, k);
                  }
               }
            }
