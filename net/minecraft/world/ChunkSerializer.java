   ...
   public static ProtoChunk deserialize(ServerWorld world, PointOfInterestStorage poiStorage, ChunkPos chunkPos, NbtCompound nbt) {
      ...
      for(int j = 0; j < lv3.size(); ++j) {
         ...
         if (l >= 0 && l < lvs.length) {
            ...
            ChunkSection lv10 = new ChunkSection(lv8, (ReadableContainer)lv9);
            lvs[l] = lv10;
         ...
         if (bl4 || bl5) {
            ...
            if (bl4) {
               lv5.enqueueSectionData(LightType.BLOCK, ChunkSectionPos.from(chunkPos, k), new ChunkNibbleArray(lv7.getByteArray("BlockLight")));
            }

            if (bl5) {
               lv5.enqueueSectionData(LightType.SKY, ChunkSectionPos.from(chunkPos, k), new ChunkNibbleArray(lv7.getByteArray("SkyLight")));
            }
         }
      }

      long m = nbt.getLong("InhabitedTime");
      ChunkStatus.ChunkType lv12 = getChunkType(nbt);
      Logger var10001;
      BlendingData lv13;
      if (nbt.contains("blending_data", NbtElement.COMPOUND_TYPE)) {
         ...
         lv13 = (BlendingData)var10000.resultOrPartial(var10001::error).orElse((Object)null);
      } else {
         lv13 = null;
      }

      Object lv16;
      if (lv12 == ChunkStatus.ChunkType.LEVELCHUNK) {
         ChunkTickScheduler lv14 = ChunkTickScheduler.create(nbt.getList("block_ticks", NbtElement.COMPOUND_TYPE), (id) -> {
            return Registries.BLOCK.getOrEmpty(Identifier.tryParse(id));
         }, chunkPos);
         ChunkTickScheduler lv15 = ChunkTickScheduler.create(nbt.getList("fluid_ticks", NbtElement.COMPOUND_TYPE), (id) -> {
            return Registries.FLUID.getOrEmpty(Identifier.tryParse(id));
         }, chunkPos);
         lv16 = new WorldChunk(world.toServerWorld(), chunkPos, lv2, lv14, lv15, m, lvs, getEntityLoadingCallback(world, nbt), lv13);
      } else {
         SimpleTickScheduler lv17 = SimpleTickScheduler.tick(nbt.getList("block_ticks", NbtElement.COMPOUND_TYPE), (id) -> {
            return Registries.BLOCK.getOrEmpty(Identifier.tryParse(id));
         }, chunkPos);
         SimpleTickScheduler lv18 = SimpleTickScheduler.tick(nbt.getList("fluid_ticks", NbtElement.COMPOUND_TYPE), (id) -> {
            return Registries.FLUID.getOrEmpty(Identifier.tryParse(id));
         }, chunkPos);
         ProtoChunk lv19 = new ProtoChunk(chunkPos, lv2, lvs, lv17, lv18, world, lv6, lv13);
         lv16 = lv19;
         lv19.setInhabitedTime(m);
         if (nbt.contains("below_zero_retrogen", NbtElement.COMPOUND_TYPE)) {
            ...
            Objects.requireNonNull(lv19);
            var33.ifPresent(lv19::setBelowZeroRetrogen);
         }
      ...
      ((Chunk)lv16).setLightOn(bl);
      NbtCompound lv21 = nbt.getCompound("Heightmaps");
      EnumSet enumSet = EnumSet.noneOf(Heightmap.Type.class);
      Iterator var46 = ((Chunk)lv16).getStatus().getHeightmapTypes().iterator();

      while(var46.hasNext()) {
         Heightmap.Type lv22 = (Heightmap.Type)var46.next();
         String string = lv22.getName();
         if (lv21.contains(string, NbtElement.LONG_ARRAY_TYPE)) {
            ((Chunk)lv16).setHeightmap(lv22, lv21.getLongArray(string));
         } else {
            enumSet.add(lv22);
         }
      }

      Heightmap.populateHeightmaps((Chunk)lv16, enumSet);
      NbtCompound lv23 = nbt.getCompound("structures");
      ((Chunk)lv16).setStructureStarts(readStructureStarts(StructureContext.from(world), lv23, world.getSeed()));
      ((Chunk)lv16).setStructureReferences(readStructureReferences(world.getRegistryManager(), chunkPos, lv23));
      if (nbt.getBoolean("shouldSave")) {
         ((Chunk)lv16).setNeedsSaving(true);
      }
      ...
      for(int n = 0; n < lv24.size(); ++n) {
         ...
         for(o = 0; o < lv25.size(); ++o) {
            ((Chunk)lv16).markBlockForPostProcessing(lv25.getShort(o), n);
         }
      }
      ...
      if (lv12 == ChunkStatus.ChunkType.LEVELCHUNK) {
         ...
         for(int p = 0; p < lv27.size(); ++p) {
            ...
            ((Chunk)lv16).addPendingBlockEntityNbt(lv28);
         }

         NbtCompound lv29 = nbt.getCompound("CarvingMasks");
         Iterator var54 = lv29.getKeys().iterator();

         while(var54.hasNext()) {
            String string2 = (String)var54.next();
            GenerationStep.Carver lv30 = GenerationStep.Carver.valueOf(string2);
            lv26.setCarvingMask(lv30, new CarvingMask(lv29.getLongArray(string2), ((Chunk)lv16).getBottomY()));
         }
