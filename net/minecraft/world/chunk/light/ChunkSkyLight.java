...
public class ChunkSkyLight {
   ...
   private int calculateSurfaceY(Chunk chunk, int topSectionIndex, int localX, int localZ) {
      ...
      BlockPos.Mutable lv2 = this.reusableBlockPos2.set(lv, (Direction)Direction.DOWN);
      ...
      for(int m = topSectionIndex; m >= 0; --m) {
         ...
         if (lv4.isEmpty()) {
            ...
            for(n = 15; n >= 0; --n) {
               ...
               if (faceBlocksLight(chunk, lv, lv3, lv2, lv5)) {
                  return lv.getY();
               }
   ...
   public boolean isSkyLightAccessible(BlockView blockView, int localX, int y, int localZ) {
      int l = y + 1;
      int m = getPackedIndex(localX, localZ);
      int n = this.get(m);
      if (l < n) {
         return false;
      } else {
         BlockPos lv = this.reusableBlockPos1.set(localX, y + 1, localZ);
         BlockState lv2 = blockView.getBlockState(lv);
         BlockPos lv3 = this.reusableBlockPos2.set(localX, y, localZ);
         BlockState lv4 = blockView.getBlockState(lv3);
         if (this.isSkyLightAccessible(blockView, m, n, lv, lv2, lv3, lv4)) {
            return true;
         } else {
            BlockPos lv5 = this.reusableBlockPos1.set(localX, y - 1, localZ);
   ...
   private int locateLightBlockingBlockBelow(BlockView blockView, BlockPos pos, BlockState blockState) {
      ...
      BlockPos.Mutable lv2 = this.reusableBlockPos2.set(pos, (Direction)Direction.DOWN);
      BlockState lv3 = blockState;

      while(lv2.getY() >= this.minY) {
         BlockState lv4 = blockView.getBlockState(lv2);
         if (faceBlocksLight(blockView, lv, lv3, lv2, lv4)) {
            return lv.getY();
         }
   ...
   public int get(int localX, int localZ) {
      int k = this.get(getPackedIndex(localX, localZ));
