      ...
      if (r >= 1 && r <= 5) {
            ...
            for(t = 3; t >= -1; --t) {
               for(u = p; u <= q; ++u) {
                  ...
                  if (s != k && t != -1 && u != p && s != l && t != 4 && u != q) {
                     if (!lv5.isOf(Blocks.CHEST) && !lv5.isOf(Blocks.SPAWNER)) {
                        this.setBlockStateIf(lv3, lv4, AIR, predicate);
                     }
                  } else if (lv4.getY() >= lv3.getBottomY() && !lv3.getBlockState(lv4.down()).isSolid()) {
                     lv3.setBlockState(lv4, AIR, Block.NOTIFY_LISTENERS);
                  } else if (lv5.isSolid() && !lv5.isOf(Blocks.CHEST)) {
         ...
         for(s = 0; s < 2; ++s) {
            for(t = 0; t < 3; ++t) {
               ...
               BlockPos lv6 = new BlockPos(u, v, w);
               if (lv3.isAir(lv6)) {
                  int x = 0;
                  Iterator var23 = Direction.Type.HORIZONTAL.iterator();

                  while(var23.hasNext()) {
                     Direction lv7 = (Direction)var23.next();
                     if (lv3.getBlockState(lv6.offset(lv7)).isSolid()) {
                        ++x;
                     }
                  }
                  ...
                  if (x == 1) {
                     ...
                     LootableContainerBlockEntity.setLootTable(lv3, lv2, lv6, LootTables.SIMPLE_DUNGEON_CHEST);
                     break;
                  }
               }
            }
