...
public class ServerPlayerEntity extends PlayerEntity {
      ...
      try {
         ...
         for(int i = 0; i < this.getInventory().size(); ++i) {
            ...
            if (lv.getItem().isNetworkSynced()) {
               Packet lv2 = ((NetworkSyncedItem)lv.getItem()).createSyncPacket(lv, this.getWorld(), this);
               if (lv2 != null) {
   ...
   private void forgiveMobAnger() {
      ...
      this.getWorld().getEntitiesByClass(MobEntity.class, lv, EntityPredicates.EXCEPT_SPECTATOR).stream().filter((entity) -> {
   ...
   protected TeleportTarget getTeleportTarget(ServerWorld destination) {
      ...
      if (lv != null && this.getWorld().getRegistryKey() == World.OVERWORLD && destination.getRegistryKey() == World.END) {
   ...
   protected Optional getPortalRect(ServerWorld destWorld, BlockPos destPos, boolean destIsNether, WorldBorder worldBorder) {
      ...
      if (optional.isPresent()) {
         ...
         Direction.Axis lv = (Direction.Axis)this.getWorld().getBlockState(this.lastNetherPortalPosition).getOrEmpty(NetherPortalBlock.AXIS).orElse(Direction.Axis.X);
   ...
   private void worldChanged(ServerWorld origin) {
      ...
      RegistryKey lv2 = this.getWorld().getRegistryKey();
   ...
   public Either trySleep(BlockPos pos) {
      Direction lv = (Direction)this.getWorld().getBlockState(pos).get(HorizontalFacingBlock.FACING);
         ...
         if (!this.getWorld().getDimension().natural()) {
            ...
            if (this.getWorld().isDay()) {
               ...
               if (!this.isCreative()) {
                  ...
                  List list = this.getWorld().getEntitiesByClass(HostileEntity.class, new Box(lv2.getX() - 8.0, lv2.getY() - 5.0, lv2.getZ() - 8.0, lv2.getX() + 8.0, lv2.getY() + 5.0, lv2.getZ() + 8.0), (entity) -> {
   ...
   public void unlockCraftedRecipe(Recipe recipe, List ingredients) {
