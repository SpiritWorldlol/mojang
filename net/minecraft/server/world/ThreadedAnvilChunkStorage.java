...
public class ThreadedAnvilChunkStorage extends VersionedChunkStorage implements ChunkHolder.PlayersWatchingChunkProvider {
   ...
   private static boolean isOnDistanceEdge(int x1, int z1, int x2, int z2, int distance) {
      if (!isWithinDistance(x1, z1, x2, z2, distance)) {
         ...
         return !isWithinDistance(x1 + 1, z1 + 1, x2, z2, distance) || !isWithinDistance(x1 - 1, z1 + 1, x2, z2, distance) || !isWithinDistance(x1 + 1, z1 - 1, x2, z2, distance) || !isWithinDistance(x1 - 1, z1 - 1, x2, z2, distance);
      }
   }
   ...
   private CompletableFuture getRegion(ChunkHolder centerChunk, int margin, IntFunction distanceToStatus) {
      if (margin == 0) {
         ChunkStatus lv = (ChunkStatus)distanceToStatus.apply(0);
         return centerChunk.getChunkAt(lv, this).thenApply((chunk) -> {
         ...
         for(int l = -margin; l <= margin; ++l) {
            for(int m = -margin; m <= margin; ++m) {
               ...
               final ChunkPos lv3 = new ChunkPos(j + m, k + l);
               long o = lv3.toLong();
               ChunkHolder lv4 = this.getCurrentChunkHolder(o);
               if (lv4 == null) {
               ...
               ChunkStatus lv5 = (ChunkStatus)distanceToStatus.apply(n);
               CompletableFuture completableFuture = lv4.getChunkAt(lv5, this);
               list2.add(lv4);
         ...
         Iterator var21 = list2.iterator();

         while(var21.hasNext()) {
            ChunkHolder lv6 = (ChunkHolder)var21.next();
            lv6.combineSavingFuture("getChunkRangeFuture " + lv2 + " " + margin, completableFuture3);
         }
   ...
   public CompletableFuture makeChunkEntitiesTickable(ChunkHolder chunk) {
      return this.getRegion(chunk, 2, (distance) -> {
   ...
   private CompletableFuture upgradeChunk(ChunkHolder holder, ChunkStatus requiredStatus) {
      ChunkPos lv = holder.getPos();
      CompletableFuture completableFuture = this.getRegion(holder, requiredStatus.getTaskMargin(), (distance) -> {
      ...
      return completableFuture.thenComposeAsync((either) -> {
         return (CompletionStage)either.map((chunks) -> {
            try {
               Chunk lvx = (Chunk)chunks.get(chunks.size() / 2);
               CompletableFuture completableFuture;
               if (lvx.getStatus().isAtLeast(requiredStatus)) {
                  completableFuture = requiredStatus.runLoadTask(this.world, this.structureTemplateManager, this.lightingProvider, (arg2) -> {
                     return this.convertToFullChunk(holder);
                  }, lvx);
               } else {
                  completableFuture = requiredStatus.runGenerationTask(executor, this.world, this.chunkGenerator, this.structureTemplateManager, this.lightingProvider, (chunk) -> {
                     return this.convertToFullChunk(holder);
                  }, chunks);
               }
               ...
               var9.getStackTrace();
               CrashReport lv2 = CrashReport.create(var9, "Exception generating new chunk");
               CrashReportSection lv3 = lv2.addElement("Chunk to be generated");
               lv3.add("Location", (Object)String.format(Locale.ROOT, "%d,%d", lv.x, lv.z));
               lv3.add("Position hash", (Object)ChunkPos.toLong(lv.x, lv.z));
               lv3.add("Generator", (Object)this.chunkGenerator);
   ...
   private CompletableFuture convertToFullChunk(ChunkHolder chunkHolder) {
      ...
      return completableFuture.thenApplyAsync((either) -> {
         ...
         return !lv.isAtLeast(ChunkStatus.FULL) ? ChunkHolder.UNLOADED_CHUNK : either.mapLeft((protoChunk) -> {
            ...
            WorldChunk lv3;
            if (lv2 instanceof WrapperProtoChunk) {
               lv3 = ((WrapperProtoChunk)lv2).getWrappedChunk();
            } else {
               lv3 = new WorldChunk(this.world, lv2, (chunk) -> {
                  addEntitiesFromNbt(this.world, lv2.getEntities());
               });
               chunkHolder.setCompletedChunk(new WrapperProtoChunk(lv3, false));
            }
   ...
   public CompletableFuture makeChunkTickable(ChunkHolder arg) {
      CompletableFuture completableFuture = this.getRegion(arg, 1, (i) -> {
         return ChunkStatus.FULL;
      });
      CompletableFuture completableFuture2 = completableFuture.thenApplyAsync((either) -> {
         ...
         this.mainExecutor.send(ChunkTaskPrioritySystem.createMessage(arg, task));
      }).thenApplyAsync((either) -> {
      ...
      completableFuture2.thenAcceptAsync((either) -> {
         either.ifLeft((chunk) -> {
            ...
            this.getPlayersWatchingChunk(arg.getPos(), false).forEach((player) -> {
   ...
   public CompletableFuture makeChunkAccessible(ChunkHolder holder) {
      return this.getRegion(holder, 1, ChunkStatus::byDistanceFromFull).thenApplyAsync((either) -> {
         return either.mapLeft((chunks) -> {
   ...
   public void updatePosition(ServerPlayerEntity player) {
      ...
      if (Math.abs(k - i) <= o * 2 && Math.abs(n - j) <= o * 2) {
         ...
         for(p = i - o; p <= i + o; ++p) {
            for(q = j - o; q <= j + o; ++q) {
               if (isWithinDistance(p, q, i, j, this.watchDistance)) {
                  bl6 = false;
                  bl7 = true;
                  this.sendWatchPackets(player, new ChunkPos(p, q), new MutableObject(), false, true);
               }
            }
         }
      }
   ...
   private void sendChunkDataPackets(ServerPlayerEntity player, MutableObject cachedDataPacket, WorldChunk chunk) {
      if (cachedDataPacket.getValue() == null) {
         cachedDataPacket.setValue(new ChunkDataS2CPacket(chunk, this.lightingProvider, (BitSet)null, (BitSet)null));
      }
