   ...
   public void tick() {
      ...
      if (lv.getType() == HitResult.Type.BLOCK) {
         ...
         if (lv3.isOf(Blocks.NETHER_PORTAL)) {
            ...
            BlockEntity lv4 = this.getWorld().getBlockEntity(lv2);
            if (lv4 instanceof EndGatewayBlockEntity && EndGatewayBlockEntity.canTeleport(this)) {
               EndGatewayBlockEntity.tryTeleportingEntity(this.getWorld(), lv2, lv3, this, (EndGatewayBlockEntity)lv4);
            }
      ...
      if (this.isTouchingWater()) {
         for(int i = 0; i < 4; ++i) {
            float g = 0.25F;
            this.getWorld().addParticle(ParticleTypes.BUBBLE, d - lv5.x * 0.25, e - lv5.y * 0.25, f - lv5.z * 0.25, lv5.x, lv5.y, lv5.z);
         }
