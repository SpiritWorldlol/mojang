...
public class PotionEntity extends ThrownItemEntity implements FlyingItemEntity {
   ...
   protected void onCollision(HitResult hitResult) {
      ...
      if (!this.getWorld().isClient) {
         ...
         this.getWorld().syncWorldEvent(i, this.getBlockPos(), PotionUtil.getColor(lv));
         this.discard();
      }
   }
   ...
   private void applyWater() {
      ...
      List list = this.getWorld().getEntitiesByClass(LivingEntity.class, lv, AFFECTED_BY_WATER);
      ...
      List list2 = this.getWorld().getNonSpectatingEntities(AxolotlEntity.class, lv);
   ...
   private void applySplashPotion(List statusEffects, @Nullable Entity entity) {
      Box lv = this.getBoundingBox().expand(4.0, 2.0, 4.0);
      List list2 = this.getWorld().getNonSpectatingEntities(LivingEntity.class, lv);
      if (!list2.isEmpty()) {
   ...
   private void extinguishFire(BlockPos pos) {
      ...
      if (lv.isIn(BlockTags.FIRE)) {
         ...
         AbstractCandleBlock.extinguish((PlayerEntity)null, lv, this.getWorld(), pos);
      } else if (CampfireBlock.isLitCampfire(lv)) {
         this.getWorld().syncWorldEvent((PlayerEntity)null, WorldEvents.FIRE_EXTINGUISHED, pos, 0);
