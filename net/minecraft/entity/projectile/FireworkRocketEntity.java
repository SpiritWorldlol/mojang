...
public class FireworkRocketEntity extends ProjectileEntity implements FlyingItemEntity {
   ...
   private void explode() {
      ...
      if (f > 0.0F) {
         ...
         List list = this.getWorld().getNonSpectatingEntities(LivingEntity.class, this.getBoundingBox().expand(5.0));
         ...
         while(true) {
            ...
            for(int i = 0; i < 2; ++i) {
               ...
               HitResult lv7 = this.getWorld().raycast(new RaycastContext(lv4, lv6, RaycastContext.ShapeType.COLLIDER, RaycastContext.FluidHandling.NONE, this));
   ...
   public void handleStatus(byte status) {
      if (status == EntityStatuses.EXPLODE_FIREWORK_CLIENT && this.getWorld().isClient) {
         if (!this.hasExplosionEffects()) {
            ...
            this.getWorld().addFireworkParticle(this.getX(), this.getY(), this.getZ(), lv3.x, lv3.y, lv3.z, lv2);
         }
      }
