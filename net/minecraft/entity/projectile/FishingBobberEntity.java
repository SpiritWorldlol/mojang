...
public class FishingBobberEntity extends ProjectileEntity {
   ...
   public void tick() {
      ...
      if (lv == null) {
         this.discard();
      } else if (this.getWorld().isClient || !this.removeIfInvalid(lv)) {
         if (this.isOnGround()) {
            ++this.removalTimer;
   ...
   private PositionType getPositionType(BlockPos pos) {
      ...
      if (!lv.isAir() && !lv.isOf(Blocks.LILY_PAD)) {
         ...
         return lv2.isIn(FluidTags.WATER) && lv2.isStill() && lv.getCollisionShape(this.getWorld(), pos).isEmpty() ? FishingBobberEntity.PositionType.INSIDE_WATER : FishingBobberEntity.PositionType.INVALID;
      } else {
   ...
   public int use(ItemStack usedItem) {
      ...
      if (!this.getWorld().isClient && lv != null && !this.removeIfInvalid(lv)) {
         ...
         if (this.hookedEntity != null) {
            ...
            LootContext.Builder lv2 = (new LootContext.Builder((ServerWorld)this.getWorld())).parameter(LootContextParameters.ORIGIN, this.getPos()).parameter(LootContextParameters.TOOL, usedItem).parameter(LootContextParameters.THIS_ENTITY, this).random(this.random).luck((float)this.luckOfTheSeaLevel + lv.getLuck());
            LootTable lv3 = this.getWorld().getServer().getLootManager().getLootTable(LootTables.FISHING_GAMEPLAY);
