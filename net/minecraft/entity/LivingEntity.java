...
public abstract class LivingEntity extends Entity implements Attackable {
            ...
            if (bl2) {
               ...
               if (this.getAir() == -20) {
                  ...
                  for(int i = 0; i < 8; ++i) {
                     ...
                     this.getWorld().addParticle(ParticleTypes.BUBBLE, this.getX() + f, this.getY() + g, this.getZ() + h, lv.x, lv.y, lv.z);
                  }
   ...
   protected void displaySoulSpeedEffects() {
      Vec3d lv = this.getVelocity();
      this.getWorld().addParticle(ParticleTypes.SOUL, this.getX() + (this.random.nextDouble() - 0.5) * (double)this.getWidth(), this.getY() + 0.1, this.getZ() + (this.random.nextDouble() - 0.5) * (double)this.getWidth(), lv.x * -0.2, 0.1, lv.z * -0.2);
      ...
      if (!bl && !ItemStack.canCombine(oldStack, newStack) && !this.firstUpdate) {
         ...
         if (lv != null && !this.isSpectator() && lv.getSlotType() == slot) {
            if (!this.getWorld().isClient() && !this.isSilent()) {
               this.getWorld().playSound((PlayerEntity)null, this.getX(), this.getY(), this.getZ(), lv.getEquipSound(), this.getSoundCategory(), 1.0F, 1.0F);
            }
   ...
   public void readCustomDataFromNbt(NbtCompound nbt) {
      ...
      if (nbt.contains("Team", NbtElement.STRING_TYPE)) {
         ...
         Team lv4 = this.getWorld().getScoreboard().getTeam(string);
         boolean bl = lv4 != null && this.getWorld().getScoreboard().addPlayerToTeam(this.getUuidAsString(), lv4);
         if (!bl) {
   ...
   protected void dropLoot(DamageSource source, boolean causedByPlayer) {
      ...
      LootTable lv2 = this.getWorld().getServer().getLootManager().getLootTable(lv);
   ...
   protected LootContext.Builder getLootContextBuilder(boolean causedByPlayer, DamageSource source) {
      LootContext.Builder lv = (new LootContext.Builder((ServerWorld)this.getWorld())).random(this.random).parameter(LootContextParameters.THIS_ENTITY, this).parameter(LootContextParameters.ORIGIN, this.getPos()).parameter(LootContextParameters.DAMAGE_SOURCE, source).optionalParameter(LootContextParameters.KILLER_ENTITY, source.getAttacker()).optionalParameter(LootContextParameters.DIRECT_KILLER_ENTITY, source.getSource());
   ...
   private boolean canEnterTrapdoor(BlockPos pos, BlockState state) {
      if ((Boolean)state.get(TrapdoorBlock.OPEN)) {
         BlockState lv = this.getWorld().getBlockState(pos.down());
      ...
      if (this.isLogicalSideForUpdatingMovement()) {
         ...
         if (this.isTouchingWater() && this.shouldSwimInFluids() && !this.canWalkOnFluid(lv)) {
            ...
            if (this.hasStatusEffect(StatusEffects.LEVITATION)) {
               ...
               this.onLanding();
            } else if (this.getWorld().isClient && !this.getWorld().isChunkLoaded(lv7)) {
   ...
   protected void tickCramming() {
      if (this.getWorld().isClient()) {
         ...
         List list = this.getWorld().getOtherEntities(this, this.getBoundingBox(), EntityPredicates.canBePushedBy(this));
         if (!list.isEmpty()) {
   ...
   protected void tickRiptide(Box a, Box b) {
      Box lv = a.union(b);
      List list = this.getWorld().getOtherEntities(this, lv);
      if (!list.isEmpty()) {
   ...
   public void wakeUp() {
      ...
      var10000.filter(var10001::isChunkLoaded).ifPresent((pos) -> {
         ...
         if (lv.getBlock() instanceof BedBlock) {
            ...
            this.getWorld().setBlockState(pos, (BlockState)lv.with(BedBlock.OCCUPIED, false), Block.NOTIFY_ALL);
