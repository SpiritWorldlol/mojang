...
public class WitherEntity extends HostileEntity implements SkinOverlayOwner, RangedAttackMob {
   ...
   public void tickMovement() {
      ...
      if (!this.getWorld().isClient && this.getTrackedEntityId(0) > 0) {
         Entity lv2 = this.getWorld().getEntityById(this.getTrackedEntityId(0));
         if (lv2 != null) {
            double d = lv.y;
         ...
         for(i = 1; i < 3; ++i) {
            if (this.age >= this.skullCooldowns[i - 1]) {
               ...
               if (j > 0) {
                  ...
                  List list = this.getWorld().getTargets(LivingEntity.class, HEAD_TARGET_PREDICATE, this, this.getBoundingBox().expand(20.0, 8.0, 20.0));
                  if (!list.isEmpty()) {
   ...
   private void shootSkullAt(int headIndex, double targetX, double targetY, double targetZ, boolean charged) {
      ...
      double l = targetY - h;
      double m = targetZ - j;
      WitherSkullEntity lv = new WitherSkullEntity(this.getWorld(), this, k, l, m);
      lv.setOwner(this);
      if (charged) {
         lv.setCharged(true);
      }
