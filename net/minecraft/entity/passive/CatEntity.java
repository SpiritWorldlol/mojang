...
public class CatEntity extends TameableEntity implements VariantHolder {
   ...
   private static class SleepWithOwnerGoal extends Goal {
      ...
      private void dropMorningGifts() {
         ...
         LootTable lv3 = this.cat.getWorld().getServer().getLootManager().getLootTable(LootTables.CAT_MORNING_GIFT_GAMEPLAY);
         LootContext.Builder lv4 = (new LootContext.Builder((ServerWorld)this.cat.getWorld())).parameter(LootContextParameters.ORIGIN, this.cat.getPos()).parameter(LootContextParameters.THIS_ENTITY, this.cat).random(lv);
         List list = lv3.generateLoot(lv4.build(LootContextTypes.GIFT));
         Iterator var6 = list.iterator();

         while(var6.hasNext()) {
            ItemStack lv5 = (ItemStack)var6.next();
            this.cat.getWorld().spawnEntity(new ItemEntity(this.cat.getWorld(), (double)lv2.getX() - (double)MathHelper.sin(this.cat.bodyYaw * 0.017453292F), (double)lv2.getY(), (double)lv2.getZ() + (double)MathHelper.cos(this.cat.bodyYaw * 0.017453292F), lv5));
         }
