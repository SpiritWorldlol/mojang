...
public abstract class PlayerEntity extends LivingEntity {
      ...
      if (entityNbt != null && (!entityNbt.contains("Silent") || !entityNbt.getBoolean("Silent")) && this.getWorld().random.nextInt(200) == 0) {
         ...
         EntityType.get(string).filter((entityType) -> {
            ...
            if (!ParrotEntity.imitateNearbyMob(this.getWorld(), this)) {
               this.getWorld().playSound((PlayerEntity)null, this.getX(), this.getY(), this.getZ(), ParrotEntity.getRandomSound(this.getWorld(), this.getWorld().random), this.getSoundCategory(), 1.0F, ParrotEntity.getSoundPitch(this.getWorld().random));
            }
         ...
         if (!target.handleAttack(this)) {
            ...
            if (f > 0.0F || g > 0.0F) {
               ...
               if (bl6) {
                  ...
                  if (!this.getWorld().isClient && !lv4.isEmpty() && lv5 instanceof LivingEntity) {
   ...
   public void addExperienceLevels(int levels) {
      ...
      if (levels > 0 && this.experienceLevel % 5 == 0 && (float)this.lastPlayedLevelUpSoundTime < (float)this.age - 100.0F) {
         ...
         this.getWorld().playSound((PlayerEntity)null, this.getX(), this.getY(), this.getZ(), SoundEvents.ENTITY_PLAYER_LEVELUP, this.getSoundCategory(), f * 0.75F, 1.0F);
   ...
   public boolean canPlaceOn(BlockPos pos, Direction facing, ItemStack stack) {
      if (this.abilities.allowModifyWorld) {
         ...
         return stack.canPlaceOn(this.getWorld().getRegistryManager().get(RegistryKeys.BLOCK), lv2);
      }
   }
