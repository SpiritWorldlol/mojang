      ...
      if (this.ambientTick < 0) {
         if (this.remainingActions == 0) {
            if (this.getWorld() instanceof ServerWorld) {
               list = this.getWorld().getOtherEntities(this, new Box(this.getX() - 15.0, this.getY() - 15.0, this.getZ() - 15.0, this.getX() + 15.0, this.getY() + 6.0 + 15.0, this.getZ() + 15.0), (arg) -> {
                  return arg.isAlive() && !this.struckEntities.contains(arg);
               });
               var2 = ((ServerWorld)this.getWorld()).getPlayers((arg) -> {
   ...
   private void spawnFire(int spreadAttempts) {
      if (!this.cosmetic && !this.getWorld().isClient && this.getWorld().getGameRules().getBoolean(GameRules.DO_FIRE_TICK)) {
         ...
         for(int j = 0; j < spreadAttempts; ++j) {
            ...
            if (this.getWorld().getBlockState(lv3).isAir() && lv2.canPlaceAt(this.getWorld(), lv3)) {
