...
public class MoveToRaidCenterGoal extends Goal {
   ...
   public boolean canStart() {
      return this.actor.getTarget() == null && !this.actor.hasPassengers() && this.actor.hasActiveRaid() && !this.actor.getRaid().isFinished() && !((ServerWorld)this.actor.getWorld()).isNearOccupiedPointOfInterest(this.actor.getBlockPos());
   }
   ...
   private void includeFreeRaiders(Raid raid) {
      if (raid.isActive()) {
         ...
         List list = this.actor.getWorld().getEntitiesByClass(RaiderEntity.class, this.actor.getBoundingBox().expand(16.0), (raider) -> {
