...
public class MoveControl implements Control {
   ...
   private boolean isPosWalkable(float x, float z) {
      ...
      if (lv != null) {
         ...
         if (lv2 != null && lv2.getDefaultNodeType(this.entity.getWorld(), MathHelper.floor(this.entity.getX() + (double)x), this.entity.getBlockY(), MathHelper.floor(this.entity.getZ() + (double)z)) != PathNodeType.WALKABLE) {
            return false;
         }
      }
