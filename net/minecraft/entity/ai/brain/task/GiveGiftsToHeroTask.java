   ...
   private List getGifts(VillagerEntity villager) {
      if (villager.isBaby()) {
         ...
         if (GIFTS.containsKey(lv)) {
            LootTable lv2 = villager.getWorld().getServer().getLootManager().getLootTable((Identifier)GIFTS.get(lv));
            LootContext.Builder lv3 = (new LootContext.Builder((ServerWorld)villager.getWorld())).parameter(LootContextParameters.ORIGIN, villager.getPos()).parameter(LootContextParameters.THIS_ENTITY, villager).random(villager.getRandom());
