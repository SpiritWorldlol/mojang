...
public class ShulkerEntity extends GolemEntity implements VariantHolder, Monster {
   ...
   private void moveEntities() {
      ...
      float h = f - g;
      if (!(h <= 0.0F)) {
         List list = this.getWorld().getOtherEntities(this, calculateBoundingBox(lv, g, f).offset(this.getX() - 0.5, this.getY(), this.getZ() - 0.5), EntityPredicates.EXCEPT_SPECTATOR.and((arg) -> {
   ...
   protected boolean tryTeleport() {
      if (!this.isAiDisabled() && this.isAlive()) {
         ...
         for(int i = 0; i < 5; ++i) {
            ...
            if (lv2.getY() > this.getWorld().getBottomY() && this.getWorld().isAir(lv2) && this.getWorld().getWorldBorder().contains(lv2) && this.getWorld().isSpaceEmpty(this, (new Box(lv2)).contract(1.0E-6))) {
               ...
               if (lv3 != null) {
                  ...
                  this.getWorld().emitGameEvent(GameEvent.TELEPORT, lv, GameEvent.Emitter.of((Entity)this));
                  this.dataTracker.set(PEEK_AMOUNT, (byte)0);
   ...
   private void spawnNewShulker() {
      ...
      if (!this.isClosed() && this.tryTeleport()) {
         ...
         if (!(this.getWorld().random.nextFloat() < f)) {
            ShulkerEntity lv3 = (ShulkerEntity)EntityType.SHULKER.create(this.getWorld());
            if (lv3 != null) {
