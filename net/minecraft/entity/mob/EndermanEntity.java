...
public class EndermanEntity extends HostileEntity implements Angerable {
   ...
   public void readCustomDataFromNbt(NbtCompound nbt) {
      ...
      if (nbt.contains("carriedBlockState", NbtElement.COMPOUND_TYPE)) {
         lv = NbtHelper.toBlockState(this.getWorld().createCommandRegistryWrapper(RegistryKeys.BLOCK), nbt.getCompound("carriedBlockState"));
         if (lv.isAir()) {
            lv = null;
         }
      }
   ...
   private boolean teleportTo(double x, double y, double z) {
      ...
      if (bl && !bl2) {
         ...
         if (bl3) {
            this.getWorld().emitGameEvent(GameEvent.TELEPORT, lv3, GameEvent.Emitter.of((Entity)this));
            if (!this.isSilent()) {
   ...
   protected void dropEquipment(DamageSource source, int lootingMultiplier, boolean allowDrops) {
      ...
      if (lv != null) {
         ...
         LootContext.Builder lv3 = (new LootContext.Builder((ServerWorld)this.getWorld())).random(this.getWorld().getRandom()).parameter(LootContextParameters.ORIGIN, this.getPos()).parameter(LootContextParameters.TOOL, lv2).optionalParameter(LootContextParameters.THIS_ENTITY, this);
   ...
   static class PlaceBlockGoal extends Goal {
      ...
      public void tick() {
         ...
         if (lv7 != null) {
            lv7 = Block.postProcessState(lv7, this.enderman.getWorld(), lv3);
