...
public class PiglinBrain {
   ...
   private static List getBarteredItem(PiglinEntity piglin) {
      LootTable lv = piglin.getWorld().getServer().getLootManager().getLootTable(LootTables.PIGLIN_BARTERING_GAMEPLAY);
      List list = lv.generateLoot((new LootContext.Builder((ServerWorld)piglin.getWorld())).parameter(LootContextParameters.THIS_ENTITY, piglin).random(piglin.getWorld().random).build(LootContextTypes.BARTER));
      return list;
   }
   ...
   private static Task makeRememberRideableHoglinTask() {
      ...
      return MemoryTransferTask.create((entity) -> {
         return entity.isBaby() && lv.shouldRun(entity.getWorld().random);
