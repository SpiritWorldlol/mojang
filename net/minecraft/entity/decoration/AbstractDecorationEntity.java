...
public abstract class AbstractDecorationEntity extends Entity {
      ...
      if (!this.getWorld().isSpaceEmpty(this)) {
         ...
         for(int k = 0; k < i; ++k) {
            for(int l = 0; l < j; ++l) {
               ...
               if (!lv4.isSolid() && !AbstractRedstoneGateBlock.isRedstoneGate(lv4)) {
                  return false;
               }
            }
         }
   ...
   public boolean handleAttack(Entity attacker) {
      if (attacker instanceof PlayerEntity lv) {
         return !this.getWorld().canPlayerModifyAt(lv, this.attachmentPos) ? true : this.damage(this.getDamageSources().playerAttack(lv), 0.0F);
      } else {
         return false;
      }
   }
