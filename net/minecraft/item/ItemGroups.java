...
public class ItemGroups {
   private static final RegistryKey BUILDING_BLOCKS = register("building_blocks");
   private static final RegistryKey COLORED_BLOCKS = register("colored_blocks");
   private static final RegistryKey NATURAL = register("natural_blocks");
   private static final RegistryKey FUNCTIONAL = register("functional_blocks");
   private static final RegistryKey REDSTONE = register("redstone_blocks");
   private static final RegistryKey HOTBAR = register("hotbar");
   private static final RegistryKey SEARCH = register("search");
   private static final RegistryKey TOOLS = register("tools_and_utilities");
   private static final RegistryKey COMBAT = register("combat");
   private static final RegistryKey FOOD_AND_DRINK = register("food_and_drinks");
   private static final RegistryKey INGREDIENTS = register("ingredients");
   private static final RegistryKey SPAWN_EGGS = register("spawn_eggs");
   private static final RegistryKey OPERATOR = register("op_blocks");
   private static final RegistryKey INVENTORY = register("inventory");
   private static final Comparator PAINTING_VARIANT_COMPARATOR = Comparator.comparing(RegistryEntry::value, Comparator.comparingInt((paintingVariant) -> {
   ...
   public static ItemGroup registerAndGetDefault(Registry registry) {
      ...
      Registry.register(registry, (RegistryKey)FUNCTIONAL, ItemGroup.create(ItemGroup.Row.TOP, 3).displayName(Text.translatable("itemGroup.functional")).icon(() -> {
         ...
         displayContext.lookup().getOptionalWrapper(RegistryKeys.PAINTING_VARIANT).ifPresent((wrapper) -> {
            addPaintings(entries, wrapper, (registryEntry) -> {
      ...
      Registry.register(registry, (RegistryKey)SEARCH, ItemGroup.create(ItemGroup.Row.TOP, 6).displayName(Text.translatable("itemGroup.search")).icon(() -> {
         ...
         while(var4.hasNext()) {
            ItemGroup lv = (ItemGroup)var4.next();
      ...
      Registry.register(registry, (RegistryKey)TOOLS, ItemGroup.create(ItemGroup.Row.BOTTOM, 0).displayName(Text.translatable("itemGroup.tools")).icon(() -> {
         ...
         entries.add((ItemConvertible)Items.FURNACE_MINECART);
         entries.add((ItemConvertible)Items.TNT_MINECART);
         displayContext.lookup().getOptionalWrapper(RegistryKeys.INSTRUMENT).ifPresent((wrapper) -> {
      ...
      Registry.register(registry, (RegistryKey)COMBAT, ItemGroup.create(ItemGroup.Row.BOTTOM, 1).displayName(Text.translatable("itemGroup.combat")).icon(() -> {
         ...
         displayContext.lookup().getOptionalWrapper(RegistryKeys.POTION).ifPresent((wrapper) -> {
            addPotions(entries, wrapper, Items.TIPPED_ARROW, ItemGroup.StackVisibility.PARENT_AND_SEARCH_TABS);
         });
      }).build());
   ...
   public static void collect() {
      ...
      do {
         ...
         ItemGroup lv2 = (ItemGroup)Registries.ITEM_GROUP.getOrThrow(lv);
         string = lv2.getDisplayName().getString();
         string2 = (String)map.put(Pair.of(lv2.getRow(), lv2.getColumn()), string);
      } while(string2 == null);
   ...
   public static ItemGroup getDefaultTab() {
      return (ItemGroup)Registries.ITEM_GROUP.getOrThrow(BUILDING_BLOCKS);
   }
   ...
   public static ItemGroup getSearchGroup() {
      return (ItemGroup)Registries.ITEM_GROUP.getOrThrow(SEARCH);
   }
