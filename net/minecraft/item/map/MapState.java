   ...
   public void update(PlayerEntity player, ItemStack stack) {
      ...
      for(int i = 0; i < this.updateTrackers.size(); ++i) {
         ...
         if (!lv2.player.isRemoved() && (lv2.player.getInventory().contains(stack) || stack.isInFrame())) {
            if (!stack.isInFrame() && lv2.player.getWorld().getRegistryKey() == this.dimension && this.showIcons) {
               this.addIcon(MapIcon.Type.PLAYER, lv2.player.getWorld(), string, lv2.player.getX(), lv2.player.getZ(), (double)lv2.player.getYaw(), (Text)null);
            }
         } else {
      ...
      if (stack.isInFrame() && this.showIcons) {
         ...
         this.addIcon(MapIcon.Type.FRAME, player.getWorld(), "frame-" + lv3.getId(), (double)lv4.getX(), (double)lv4.getZ(), (double)(lv3.getHorizontalFacing().getHorizontal() * 90), (Text)null);
      ...
      if (lv7 != null && lv7.contains("Decorations", NbtElement.LIST_TYPE)) {
         ...
         for(int j = 0; j < lv8.size(); ++j) {
            ...
            if (!this.icons.containsKey(lv9.getString("id"))) {
               this.addIcon(MapIcon.Type.byId(lv9.getByte("type")), player.getWorld(), lv9.getString("id"), lv9.getDouble("x"), lv9.getDouble("z"), lv9.getDouble("rot"), (Text)null);
            }
         }
      }
