...
public class PitcherCropBlock extends TallPlantBlock implements Fertilizable {
   ...
   public VoxelShape getOutlineShape(BlockState state, BlockView world, BlockPos pos, ShapeContext context) {
      return state.get(HALF) == DoubleBlockHalf.UPPER ? UPPER_OUTLINE_SHAPES[Math.min(Math.abs(4 - ((Integer)state.get(AGE) + 1)), UPPER_OUTLINE_SHAPES.length - 1)] : LOWER_OUTLINE_SHAPES[(Integer)state.get(AGE)];
   }
